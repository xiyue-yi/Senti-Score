...
def get_vary_recalls(all_num,pos_neg,flag):
	...
	p_min = pos_neg/(pos_neg+1)
	p_max = 0.95
	
	precision_list = list(i/1000 for i in range(600,950,1))
	...
	for precision_vary in precision_list:
		...
		for accuracy_vary in accuracy_list:
			...
			if (precision_vary<precision_min) or (precision_vary>precision_max) or (accuracy_vary<a_min) or (accuracy_vary>a_max):
				error_list.append(0)
				rate_list.append(0)
				continue
			...
			groundtruth_score = new_pos_neg*(2.0*accuracy/(2.0*recall-1.0)-1.0) + ((2.0*accuracy-2.0)/(2.0*recall-1.0)-1.0)
			groundtruth_score_2 = pos_neg*(2.0*accuracy_vary/(2.0*recall_vary-1.0)-1.0) + ((2.0*accuracy_vary-2.0)/(2.0*recall_vary-1.0)-1.0)
			
			print('groundtruth_score',groundtruth_score,'r',recall,'a',accuracy)
			predict_diata = abs(groundtruth_score-new_pos_neg+1)
			...
			for i in range(10000):
				...
				edit_score = 2*recall_1*new_pos_neg - 2*recall_2 + 1 - new_pos_neg
				...
				#print(recall_1,recall_2,diata_score,'tn',tn,'fp',fp,'fn',fn,'tp',tp)
			...
			print('total',total_score,total_score_2,'better',better_score,'r',recall_vary,'a',accuracy_vary,'\n')
			...
			#n, bins, patches=plt.hist(np_accuracies,6,density=False,histtype='stepfilled',facecolor='r',alpha=0.8)
	...
	errorframe.to_csv("edit_error.csv",index=False,sep=',')
	rateframe = pd.DataFrame(rate_dict)
	rateframe.to_csv("edit_rate.csv",index=False,sep=',')
