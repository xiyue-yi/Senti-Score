...
def get_vary_recalls(all_num,pos_neg,flag,part_index):
	...
	precision_list = list(i/1000 for i in range(910,920,1))
	#precision_list = [0.91]
	p_2=0.9
	...
	for precision_vary in precision_list:
		...
		#fn>0 max
		a_max = (2*precision_vary*pos_neg-pos_neg+precision_vary)/((pos_neg+1)*precision_vary)
		#a_max=0.95
		print('precision',precision_vary,'a_min',a_min,'a_max',a_max)
		#accuracy_list = [0.834]
		#0.878
		#accuracy_list = list(i/1000 for i in range(int(a_min*1000),int(a_max*1000),1))
		accuracy_list = list(i/1000 for i in range(940,950,1))
		...
		for accuracy_vary in accuracy_list:
			...
			predict_diata = abs(predict_score-pos_neg+1)
			...
			for i in range(10000):
				...
				diata_score = abs(edit_score-pos_neg+1)
	...
	errorframe.to_csv("edit_error"+str(part_index)+".csv",index=False,sep=',')
	rateframe = pd.DataFrame(rate_dict)
	rateframe.to_csv("edit_rate"+str(part_index)+".csv",index=False,sep=',')
