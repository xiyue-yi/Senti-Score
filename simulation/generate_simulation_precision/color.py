...
def draw_error():
	data=pd.read_csv("new_error.csv",header=0)
	...
	norm = colors.Normalize(vmin=-400,vmax=400)
	...
	#'YlGn'
	#plt.get_cmap("PiYG", 7)
	plt.colorbar(ax.imshow(data,cmap='YlGn',norm=norm,extent=(0.9, 0.975, 0.9, 0.975)))
...
def draw_rate():
	data=pd.read_csv("new_rate.csv",header=0)
	 #必须添加header=None，否则默认把第一行数据处理成列名导致缺失
	data_ls=data.values.tolist()
	data = np.array(data_ls)
	...
	fig, ax = plt.subplots()
	plt.colorbar(ax.imshow(data, cmap = cm.coolwarm, extent=(0.9, 0.975, 0.9, 0.975)))
