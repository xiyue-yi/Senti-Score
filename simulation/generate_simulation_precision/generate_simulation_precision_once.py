...
def get_vary_recalls(all_num,pos_neg,flag,part_index):
	...
	#precision_list = list(i/1000 for i in range(988,995,1))
	precision_list = [0.974]
	p_2=0.5
	...
	for precision_vary in precision_list:
		...
		#accuracy_list = list(i/1000 for i in range(900,995,5))
		accuracy_list = [0.961]
		for accuracy_vary in accuracy_list:
			...
			for i in range(50):
				...
				diata_score = abs(edit_score-groundtruth)
				#diata_score_2 = abs(edit_score_2 - groundtruth_score_2)
				total_score += diata_score
				#total_score_2 += diata_score_2
				print(edit_score,diata_score,precision,accuracy,tp/(tp+fp),(tp+tn)/(tp+tn+fp+fn),tp+fn-tn-fp)
				recall = recall_1
