...
def generate_simulation(pos_num,rate,recall,recall_2):

	#pos_num = int(total_num*rate/(1+rate))
	neg_num = int(pos_num/rate)
...
def get_vary_recalls(pos_num,pos_neg,flag):
	recall_vary_list = [0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95]
	record_filename = str(pos_num) + '_' + str(pos_neg)+'.txt'
	...
	for recall_1_vary in recall_1_list:
		...
		for recall_2_vary in recall_2_list:
			tp,fn,fp,tn = generate_simulation(pos_num,pos_neg,recall_1_vary,recall_2_vary)
		...
		#for rate in range(5,20):
			...
			#plt.hist(recall_2s,20,density=1,histtype='stepfilled',facecolor='b',alpha=0.2)
...
def main():
	...
	for pos_num in [1000,2000,3000,4000]:
		for pos_neg in [2,1,0.5]:		
			get_vary_recalls(pos_num,pos_neg,0)
			get_vary_recalls(pos_num,pos_neg,1)
