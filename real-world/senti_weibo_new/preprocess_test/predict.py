...
parser.add_argument("--load-model", type=str, default="saved_model/1/ckpt-epoch-8")
...
def predict(args):
    ...
    for test_dataloader,file_name in test_dataloader_list:
        result_name = os.path.join(args.test_result,file_name+'tmp.txt')
        if os.path.exists(result_name):
            print(result_name+'already exists!')
            continue
        print("Begin predict date",file_name)
        #prediction, id  = trainer.predict(test_dataloader)
        outputs, id, pred = trainer.predict(test_dataloader)

        '''
        with open(result_name, "w", encoding="utf-8") as f:
            for index in range(len(prediction)):

                f.write("{}\t{}\n".format(id[index], prediction[index].numpy()))
        f.close()
        '''
        np.savetxt(result_name, prediction)
