...
def kmeans(data,times,num):
    ...
    #for i in range(times):
        #num = 15
        x.append(num)
        clf = MiniBatchKMeans(n_clusters=num)
        s = clf.fit(data)
...
def write_kmeans(data,y_pred,y_ground,y_pos,num):
    labels,centres,distance = kmeans(data,1000,num)
...
def cal_sample(data,name1,name2):
    ...
    #p_samples,p_distance = write_kmeans(p_data,p_y_pred,p_y_ground,pos_index,15)
    #write_kmeans(n_data,n_y_pred,n_y_ground,neg_index)
    n_samples,n_distance = write_kmeans(n_data,n_y_pred,n_y_ground,neg_index,10)
