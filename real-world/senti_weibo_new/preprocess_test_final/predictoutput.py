...
from trainoutput import Trainer
...
def predict(args):
    ...
    for test_dataloader,file_name in test_dataloader_list:
        ...
        output_name = os.path.join(args.test_result,file_name+'tmp.txt')
        if os.path.exists(result_name):
            print(result_name+'already exists!')
            continue
        print("Begin predict date",file_name)
        #prediction, id  = trainer.predict(test_dataloader)
        outputs, id, prediction = trainer.predict(test_dataloader)
        ...
        np.savetxt(output_name, outputs)
