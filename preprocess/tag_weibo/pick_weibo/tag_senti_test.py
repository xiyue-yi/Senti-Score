...
tag_file = os.path.join(topic_name,'2019-06-08.txt')
record_file = os.path.join(topic_name,'num.txt')
tagged_json = os.path.join(topic_name,'tagged.json')
f_tagged_json = open(tagged_json,'a',encoding="utf-8")

neu_json = os.path.join(topic_name,'neu.json')
f_neu_json = open(neu_json,'a',encoding="utf-8")

uless_json = os.path.join(topic_name,'uless.json')
...
class TAG_GUI():
	def __init__(self,init_window_name,topic_name):
		...
		self.set_init_num()
		self.next_str = self.weibo_data[self.cur_index]
	...
	def init_weibo_data(self):
		tag_file = os.path.join(self.topic,'2019-06-08.txt')
...
def gui_start():
	init_window = Tk()
	tag_gui = TAG_GUI(init_window,topic_name)
